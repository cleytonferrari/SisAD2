{
 Sistema de controle de segurança desenvolvido por
 cleyton cesar ferrari.
...
procedure TfrmProt2003.Executa(Arquivo: String; Estado: Integer);
var
...
procedure TfrmProt2003.FormCreate(Sender: TObject);
var
ArqIni : TIniFile;
...
begin
  ...
  IconeBarra.szTip := 'ProtSisAD2'; //Hint do icone
...
procedure TfrmProt2003.FormDestroy(Sender: TObject);
begin
  IconeBarra.uFlags := 0;
  Shell_NotifyIcon (NIM_DELETE, @IconeBarra);
end;

procedure TfrmProt2003.FormShow(Sender: TObject);
var
H : HWnd;
begin
 H := FindWindow(Nil,'ProtSisAD2');
 if H <> 0 then
...
procedure TfrmProt2003.Relogio01Timer(Sender: TObject);
var
 ArqIni : TIniFile;
 ...
 //*************************
  ...
  begin
   ...
   //MARQUE AQUI A QUANTIDADE DE DIAS PARA VALIDAR O SISTEMA
     ArqIni.WriteString('P4','D.F',Criptografia(DateToStr(Date+10)));
   //#################################################################
...
procedure TfrmProt2003.Timer1Timer(Sender: TObject);
Var
  ArqIni : TIniFile;
begin
  timer1.Interval := 0;
  ...
  Executa('C:\SisAD2\SisAD2.exe 111181',1);
  end else begin
    ArqIni.Free;
    Application.MessageBox(
      'A Licença Venceu contacte o Suporte Tecnico.',
      'SisAD2',MB_OK+mb_iconinformation );
  end;
  //#########################################################
  close;
end;
